...
namespace day4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(Passport.PassportData.Count);
            var batch = new PassportBatch
            {
                BatchData = Passport.PassportData,
                requiredFields = new Dictionary<string, IField>{
                    {
                        "byr",
                        new rangeField
                        {
                    ...
                    {
                        "hcl",
                        new ColourField{Name = "hcl"}
                    },
                    {
                        "ecl",
                        new ColourField{Name = "ecl"}
                    },
                    {
                        "pid",
                        new IdField()
            ...
            var validItems = Passport.ValidatePassportData(new List<PassportBatch> { batch });
            Console.WriteLine(validItems[0]);

            var validFieldItems = Passport.ValidateFields(new List<PassportBatch> { batch });
            Console.WriteLine(validFieldItems[0]);

        }   
    }

    public class rangeField : IField
    {
        ...
        {
            var isValid = false;
            if (Int32.TryParse(value, out var numVal))
            {
                isValid =  numVal >= rangeStart && numVal <= rangeEnd;
            }
            return isValid;
        }
    }

    public class HeightField : IField
    {
        public string Name { get; set; } = "hgt";
        ...
        public bool Validator(string value)
        {
            ...
            if (value.Contains("cm") || value.Contains("in"))
            {
                ...
                {
                    if (value.Contains("cm")) isValid = result >= 150 && result <= 193;
                    if (value.Contains("in")) isValid = result >= 59 && result <= 76;
                }
            }
            return isValid;
        }
    ...
    public class ColourField : IField
    {
        ...
        public bool Validator(string value)
        {
            ...
            if (Name == "ecl")
            {
                ...
                isValid = validColors.Contains(value);
            }
            else if (Name == "hcl")
            {
                var pattern = "^#[a-f0-9]{6}";
                Match m = Regex.Match(value, pattern);
                if (m.Success) isValid = true;
            }
            return isValid;
        }
    }
    ...
    {
        public string Name { get; set; } = "pid";

        public bool Validator(string value)
        {
            var isValid = false;
            if (Int64.TryParse(value, out var num))
            {
    ...
    {
        public Dictionary<string, IField> requiredFields { get; set; }
    ...
    public static class Passport
    {
        ...
        public static List<int> ValidateFields(List<PassportBatch> data)
        {
            var validPassportItems = 0;
            var validFields = new List<string>();
            foreach (var batch in data)
            {
                foreach (var item in batch.BatchData)
                {
                    var validItem = false;
                    string cleaned = Regex.Replace(item, @"\t|\n|\r", " ").Replace(" ", "|").Replace("||", "|");
                    var fields = cleaned.Split("|")
                    .Select(x => (x.Split(":")[0], x.Split(":")[1]))
                    .ToList();
                    if (batch.requiredItems.All(x => fields.Select(y => y.Item1).ToList().Contains(x)))
                    {
                        foreach (var field in fields)
                        {
                            if (batch.requiredFields.ContainsKey(field.Item1))
                            {
                                var fld = batch.requiredFields[field.Item1];
                                validItem = fld.Validator(field.Item2);
                                if(validItem) validFields.Add($"{field.Item1} : {field.Item2}" );
                                if(!validItem) break;
                            }
                        }
                    }
                    if (validItem) validPassportItems++;
                }
            }
            ...
            using (StreamWriter outputFile = new StreamWriter(Path.Combine(docPath, "validFields.txt")))
            {
                foreach (string line in validFields)
                    outputFile.WriteLine(line);
            }

            return new List<int> { validPassportItems };
        }

        public static List<int> ValidatePassportData(List<PassportBatch> data)
        {
            var validPassportItems = 0;
            foreach (var batch in data)
            {
                foreach (var item in batch.BatchData)
                {
                    ...
                    .Select(x => x.Split(":")[0])
                    .ToList();
                    if (batch.requiredItems.All(x => fields.Contains(x))) validPassportItems++;
                    //Console.WriteLine(cleaned);
                }
            }
            return new List<int> { validPassportItems };
        }
        public static List<string> PassportData = new List<string>
        {
@"eyr:2029 iyr:2013
