...
namespace day7
{
    class Program
    {
        ...
        {
            // var bagAnalyser = BagFinder.GetBagCombos("shiny gold bags", Problem.BagList);
            // BagFinder.CheckBags(bagAnalyser);
            // Console.WriteLine(bagAnalyser.BagComboCount);

            var bagAnalyser = BagFinder.GetBagsWithCount(Problem.BagList);
            bagAnalyser.BagsToHold = new List<(string, int)> { ("shiny gold bag", 1) };
            BagFinder.CountBags(bagAnalyser);
            Console.WriteLine(bagAnalyser.InternalBagsCount);

        }

        public static class BagFinder
        {
            public static BagAnalyser GetBagCombos(string bagType, List<string> bagDescriptions)
            {
                var bags = new Dictionary<string, List<string>>();

                var bagsThatHoldType = new List<string>();

                foreach (var item in bagDescriptions)
                {
                    var containSplit = item.Split("contain");
                    var bag = containSplit[0];
                    if (bag.Contains(bagType))
                    {
                        bagsThatHoldType.Add(bag.Trim().Replace("bags", "bag"));
                    }
                    var internalBags = containSplit[1].Split(",");
                    if (internalBags.Contains("contain no other bags")) continue;
                    var internalBagsList = new List<string>();
                    foreach (var intBag in internalBags)
                    {
                        internalBagsList.Add(Regex.Replace(intBag, @"\d", "").Trim().Replace(".", "").Replace(",", "").Replace("bags", "bag"));
                    }
                    bags.Add(bag.Trim().Replace("bags", "bag"), internalBagsList);
                }

                var bagAnalyser = new BagAnalyser
                { Bags = bags, BagsThatHold = bagsThatHoldType, BagsAdded = true, BagComboCount = 0 };

                return bagAnalyser;
            }

            public static BagAnalyser GetBagsWithCount(List<string> bagDescriptions)
            {
                ...
                {
                    var containSplit = item.Split("contain");
                    var bag = containSplit[0].Trim().Replace("bags", "bag");


                    var internalBags = containSplit[1].Split(",");
                    var internalBagsList = new Dictionary<string, int>();
                    if (Regex.Match(internalBags[0], "no other bag").Success)
                    {
                        bags.Add(bag, internalBagsList);
                        continue;
                    }

                    foreach (var intBag in internalBags)
                    {
                        var bagName = Regex.Replace(intBag, @"\d", "").Trim().Replace(".", "").Replace(",", "").Replace("bags", "bag");
                        int.TryParse(Regex.Match(intBag, @"\d").Value, out var bagCount);
                        internalBagsList.Add(bagName, bagCount);
                    }
                    bags.Add(bag, internalBagsList);
                }

                var bagAnalyser = new BagAnalyser
                { BagsWithCount = bags, BagsAdded = true, InternalBagsCount = 0 };
            ...
            {
                public Dictionary<string, List<string>> Bags { get; set; }

                public Dictionary<string, Dictionary<string, int>> BagsWithCount { get; set; }

                public List<string> BagsThatHold { get; set; }

                public List<(string, int)> BagsToHold { get; set; }

                public bool BagsAdded { get; set; }

                public int BagComboCount { get; set; }

                public int InternalBagsCount { get; set; }
            }
            ...
            public static void CheckBags(BagAnalyser bagAnalyser)
            {
                var anyBagsAdded = false;

                var newBags = new Dictionary<string, List<string>>();
                var newBagsThatHold = new List<string>();
                while (bagAnalyser.BagsAdded)
                {
                    foreach (var item in bagAnalyser.Bags)
                    {
                        var hadRightBag = false;
                        foreach (var internalBag in item.Value)
                        {
                            if (bagAnalyser.BagsThatHold.Contains(internalBag))
                            {
                                if (!newBagsThatHold.Contains(item.Key)) newBagsThatHold.Add(item.Key);
                                anyBagsAdded = true;
                                hadRightBag = true;
                            }
                        }
                        if (!hadRightBag) newBags.Add(item.Key, item.Value);
                    }
                    bagAnalyser.BagComboCount = bagAnalyser.BagComboCount + newBagsThatHold.Count;
                    bagAnalyser.BagsThatHold = newBagsThatHold;
                    bagAnalyser.Bags = newBags;
                    bagAnalyser.BagsAdded = anyBagsAdded;
                    CheckBags(bagAnalyser);
                }
                return;
            }

            public static void CountBags(BagAnalyser bagAnalyser)
            {
                var anyBagsAdded = false;

                var newBagsToHold = new List<(string, int)>();
                while (bagAnalyser.BagsAdded)
                {
                    foreach (var (name, count) in bagAnalyser.BagsToHold)
                    {
                        var bags = bagAnalyser.BagsWithCount[name];

                        foreach (var item in bags)
                        {
                            anyBagsAdded = true;
                            bagAnalyser.InternalBagsCount += (count * item.Value);
                            newBagsToHold.Add((item.Key, count*item.Value));
                        }
                    }
                    bagAnalyser.BagsAdded = anyBagsAdded;
                    bagAnalyser.BagsToHold = newBagsToHold;
                    CountBags(bagAnalyser);
                }
                return;
            }
        }
        ...
        public static class Problem
        {
            ...
            // public static List<string> BagList = new List<string>
            // {
