        ...
        public static class BagFinder
        {
            public static int GetBagCombos(string bagType, List<string> bagDescriptions)
            {
                ...
                foreach (var item in bagDescriptions)
                {
                    ...
                    {
                        if (intBag.Contains(bagType))
                        {
                            bagsThatHoldType.Add(bag.Trim().Replace(".", ""));
                            holdsType = true;
                        }
                        else
                        {
                            internalBagsList.Add(Regex.Replace(intBag, @"\d", "").Trim().Replace(".", "").Replace(",", ""));
                        }
                    }
                    if (holdsType) continue;
                    bags.Add(bag.Trim(), internalBagsList);
                }

                var bagAnalyser = new BagAnalyser
                { bags = bags, bagsThatHold = bagsThatHoldType, bagsAdded = true, bagCount = bagsThatHoldType.Count };
