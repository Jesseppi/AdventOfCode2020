...
namespace day7
{
    class Program
    {
        ...
        {
            var count = BagFinder.GetBagCombos("shiny gold", Problem.BagList);
        ...
        public static class BagFinder
        {
            public static int GetBagCombos(string bagType, List<string> bagDescriptions)
            {
                ...
                CheckBags(bags, bagsThatHoldType, bagsThatHoldType.Count);

                return bagsThatHoldType.Distinct().Count();
            }

            private static void CheckBags(Dictionary<string, List<string>> bags, List<string> bagsThatHoldType, int bagTypeCount)
            {
                var bagAdds = 0;
                ...
                {
                    var canHoldType = false;
                    foreach (var internalBag in item.Value)
                    {
                        if (bagsThatHoldType.Contains(internalBag))
                        {
                            canHoldType = true;
                        }
                    }
                    var strippedItem = item.Key.Trim().Replace(".", "");
                    if (canHoldType)
                    {
                        bagAdds++;
                        if (!bagsThatHoldType.Contains(strippedItem)) bagsThatHoldType.Add(strippedItem);
                        if (!canHoldType) newBags.Add(item.Key, item.Value);
                    }
                }
