...
namespace day6
{
    class Program
    {
        ...
        {
            Console.WriteLine(AnswerCounter.CountAnswers(new List<List<string>> { CustomsDeclaration.Answers })[0]);
        }

        public class AnswerCounter
        {
            public static List<int> CountAnswers(List<List<string>> answers)
            {
                var listCounts = new List<int>();
                foreach (var list in answers)
                {
                    ...
                    foreach (var item in list)
                    {
                        if (!string.IsNullOrWhiteSpace(item))
                        {
                            ...
                            {
                                if (!groupAnswers.ContainsKey(character))
                                {
                                    groupAnswers.Add(character, 1);
                                }
                                else
                                {
                                    var currentCharCount = groupAnswers[character];
                                    currentCharCount++;
                                    groupAnswers[character] = currentCharCount;
                                };
                            }
                        }
                        else
                        {
                            var unanimousAnswers = 0;
                            foreach (var answer in groupAnswers)
                            {
                                if (answer.Value == memberCount) unanimousAnswers++;
                            }

                            listCount.Add(unanimousAnswers);
                            memberCount = 0;
                            groupAnswers = new Dictionary<char, int>();
                        }
                    }
                    var sum = listCount.Take(listCount.Count).Sum();
                    listCounts.Add(sum);
                }
                return listCounts;
